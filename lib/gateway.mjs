...
function buildModelsPayload(config) {
  const modelMap = new Map(); // id → {id, object, owned_by, aliases: []}

  for (const upstream of config.upstreams) {
    const supportedModels = upstream.supported_models ||
                           [...new Set(Object.values(upstream.model_map || {}))];
    ...
    // 收集别名
    for (const [alias, actualModel] of Object.entries(aliases)) {
      if (modelMap.has(actualModel)) {
        const model = modelMap.get(actualModel);
  ...
  return { object: 'list', data: Array.from(modelMap.values()) };
}
...
export class GatewayApp {
  ...
  async init() {
    ...
    this.upstreamManager.hydrate(await this.database.getUpstreamStates());
  ...
  buildRoutePlan(protocol, requestBody, listener) {
    ...
    if (allRoutes.length > 1) {
      this.logger.debug('route plan built', {
        app_name: listener.app_name,
        protocol,

...
function createEmptyUpstreamSummary(upstreamId) {
  return {
...
function mergeUpstreamSummaryRows(successRows = [], failedAttemptRows = []) {
  const byUpstream = new Map();
  const ensureRow = (upstreamId) => {
    if (!upstreamId) {
      return null;
    }
    if (!byUpstream.has(upstreamId)) {
      byUpstream.set(upstreamId, createEmptyUpstreamSummary(upstreamId));
    }
    return byUpstream.get(upstreamId);
  };

  for (const row of successRows) {
    const upstreamId = normalizeNullableString(row.upstream_id);
    const target = ensureRow(upstreamId);
    if (!target) {
      continue;
    }
    const successfulRequests = Number(row.successful_requests || row.total_requests || 0);
    target.total_requests += successfulRequests;
    target.successful_requests += successfulRequests;
    target.tokens_total += Number(row.tokens_total || 0);
    target.total_cost_usd += Number(row.total_cost_usd || 0);
  }
  ...
  for (const row of failedAttemptRows) {
    ...
    for (const attempt of attempts) {
      const upstreamId = normalizeNullableString(attempt?.upstream_id);
      const target = ensureRow(upstreamId);
      if (!target) {
        continue;
      }
  ...
  return [...byUpstream.values()].sort((left, right) => (
    Number(right.total_cost_usd || 0) - Number(left.total_cost_usd || 0)
      || Number(right.total_requests || 0) - Number(left.total_requests || 0)
      || String(left.upstream_id).localeCompare(String(right.upstream_id))
  ));
}

async function queryPostgresUpstreamSummary(pool, since) {
  const [successRows, failedAttemptRows] = await Promise.all([
    pool.query(
      `
        SELECT
          upstream_id,
  ...
  return mergeUpstreamSummaryRows(successRows.rows, failedAttemptRows.rows);
}

function querySqliteUpstreamSummary(db, since) {
  ...
  const failedAttemptRows = db.prepare(`
    SELECT metadata_json
    FROM requests
    WHERE timestamp >= ?
      AND metadata_json IS NOT NULL
  ...
  return mergeUpstreamSummaryRows(successRows, failedAttemptRows);
}
...
export class DatabaseAdapter {
  ...
  async initSqlite() {
    ...
    this.insertStateStatement = this.db.prepare(`
      INSERT INTO upstream_states (
        ...
        average_latency_ms, success_count, failure_count, last_error, last_failure_error, updated_at
      ) VALUES (
        ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
      )
  ...
  ensureSqliteSchema() {
    const columns = this.db.prepare('PRAGMA table_info(upstream_states)').all();
    const columnNames = new Set(columns.map((column) => column.name));
    if (!columnNames.has('last_failure_error')) {
      this.db.exec('ALTER TABLE upstream_states ADD COLUMN last_failure_error TEXT');
    }
    this.db.exec(`
      UPDATE upstream_states
      SET last_failure_error = last_error
      WHERE last_failure_error IS NULL
  ...
  async ensurePostgresSchema() {
    const result = await this.pool.query(`
      ...
      WHERE table_name = 'upstream_states'
    `);
    const columnNames = new Set(result.rows.map((row) => row.column_name));
    if (!columnNames.has('last_failure_error')) {
      await this.pool.query('ALTER TABLE upstream_states ADD COLUMN last_failure_error TEXT');
    }
  ...
  async getSummary(hours = 24) {
    ...
    if (this.pool) {
      const [summary, apps, upstreams, trend] = await Promise.all([
        ...
        queryPostgresUpstreamSummary(this.pool, since),
        this.pool.query(
          `
            SELECT
    ...
    const upstreams = querySqliteUpstreamSummary(this.db, since);
  ...
  async getUpstreamStates() {
    ...
    if (this.pool) {
      ...
      return result.rows.map((row) => ({
        ...
        cooldown_until: row.cooldown_until ? Number(row.cooldown_until) : null,
        last_failure_at: row.last_failure_at ? Number(row.last_failure_at) : null,
        last_success_at: row.last_success_at ? Number(row.last_success_at) : null,
    ...
    return this.db.prepare('SELECT * FROM upstream_states ORDER BY upstream_id ASC').all().map((row) => ({
      ...row,
      consecutive_failures: Number(row.consecutive_failures || 0),
      cooldown_until: row.cooldown_until ? Number(row.cooldown_until) : null,
      last_failure_at: row.last_failure_at ? Number(row.last_failure_at) : null,
